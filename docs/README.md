...
Repository Markdown remains the source of truth. The site is generated by `scripts/build_docs.py`, uses local search, and loads no analytics or third-party runtime scripts. The public URL requires **Settings → Pages → Source: GitHub Actions**; until that owner setting is enabled, use the Markdown files directly.
...
- [Async and streaming API](ASYNC_STREAMING.md) — non-blocking execution, normalized SSE streams, partial results, timeouts, and cancellation.
...
- [Public workload suite](WORKLOADS.md) — five synthetic context shapes, four profiles, 20 deterministic routing checks, and fixture protocol.
...
- [GitHub setup](GITHUB_SETUP.md) — repository presentation, Pages, environments, and governance settings.
...
7. offline workload estimates must never be presented as provider billing or model-quality evidence.
